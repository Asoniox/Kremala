...
def checkWord(letter):
    ...
    if img_index==7: # Ο παίκτης έκανε ανεπιτυχή προσπάθεια και δυστυχώς κρεμάστηκε
        DisableLettersBtn("All")
        messagebox.showinfo("Παιχνίδι Κρεμάλα","Δυστυχώς δε μπόρεσες να βρεις τη λέξη!\nΗ λέξη είναι: " + HiddenWord)

# Συνάρτηση η οποία εκτελείται όταν χρήστης κάνει κλικ στο κουμπί "Βρήκα τη λέξη"
def KnowWord():
    global DisplayWord
    global HiddenWord
    answer = simpledialog.askstring("Βρήκα τη λέξη", "Πιστεύεις ότι βρήκες τη λέξη!\nΓράψε τη λέξη με κεφαλαία ελληνικά χωρίς τονισμό.",parent=root)
    if answer==HiddenWord:
       messagebox.showinfo("Παιχνίδι Κρεμάλα","Μπράβο βρήκες τη λέξη!")
       DisableLettersBtn("All")
       DisplayWord.set(HiddenWord)
    elif not (answer==None) and not (answer==""):
       messagebox.showinfo("Παιχνίδι Κρεμάλα","Λυπάμαι! Δεν είναι αυτή η λέξη. Συνέχισε τη προσπάθεια.")
    else:
        return
...
def GiveUp():
    global img_index
    MsgBox = messagebox.askquestion ('Επιβεβαίωση παραίτησης','Ζήτησες παραίτηση. Να συνεχίσω;',icon = 'warning')
    if MsgBox == 'yes':
        img_index=7
        showImg()
        DisableLettersBtn("All")
        messagebox.showinfo("Παιχνίδι Κρεμάλα","Δυστυχώς δήλωσες παραίτηση!\nΗ λέξη είναι: " + HiddenWord)
...
def readfile():
    WordsList=[]
    dataFile=open("hangman_words.txt","r", encoding="utf8")
...
def showLettersBtn():
    ...
    global BtnA
    BtnA=Button(frameBtn,text="Α",state=DISABLED,width=3,command=lambda:checkWord("Α"))
    BtnA.grid(row=0,column=0, sticky=W)
    global BtnB
    BtnB=Button(frameBtn,text="B",state=DISABLED,width=3,command=lambda:checkWord("Β"))
    BtnB.grid(row=0,column=1, sticky=W)
    global BtnGama
    BtnGama=Button(frameBtn,text="Γ",state=DISABLED,width=3,command=lambda:checkWord("Γ"))
    BtnGama.grid(row=0,column=2, sticky=W)
    global BtnDelta
    BtnDelta=Button(frameBtn,text="Δ",state=DISABLED,width=3,command=lambda:checkWord("Δ"))
    BtnDelta.grid(row=0,column=3, sticky=W)
    global BtnE
    BtnE=Button(frameBtn,text="Ε",state=DISABLED,width=3,command=lambda:checkWord("Ε"))
    BtnE.grid(row=0,column=4, sticky=W)
    global BtnZ
    BtnZ=Button(frameBtn,text="Z",state=DISABLED,width=3,command=lambda:checkWord("Ζ"))
    BtnZ.grid(row=0,column=5, sticky=W)
    global BtnH
    BtnH=Button(frameBtn,text="H",state=DISABLED,width=3,command=lambda:checkWord("Η"))
    BtnH.grid(row=0,column=6, sticky=W)
    global BtnThita
    BtnThita=Button(frameBtn,text="Θ",state=DISABLED,width=3,command=lambda:checkWord("Θ"))
    BtnThita.grid(row=0,column=7, sticky=W)
    global BtnI
    BtnI=Button(frameBtn,text="I",state=DISABLED,width=3,command=lambda:checkWord("Ι"))
    BtnI.grid(row=0,column=8, sticky=W)
    global BtnK
    BtnK=Button(frameBtn,text="K",state=DISABLED,width=3,command=lambda:checkWord("Κ"))
    BtnK.grid(row=0,column=9, sticky=W)
    global BtnL
    BtnL=Button(frameBtn,text="Λ",state=DISABLED,width=3,command=lambda:checkWord("Λ"))
    BtnL.grid(row=0,column=10, sticky=W)
    global BtnM
    BtnM=Button(frameBtn,text="M",state=DISABLED,width=3,command=lambda:checkWord("Μ"))
    BtnM.grid(row=0,column=11, sticky=W)
    global BtnN
    BtnN=Button(frameBtn,text="N",state=DISABLED,width=3,command=lambda:checkWord("Ν"))
    BtnN.grid(row=1,column=0, sticky=W)
    global BtnXi
    BtnXi=Button(frameBtn,text="Ξ",state=DISABLED,width=3,command=lambda:checkWord("Ξ"))
    BtnXi.grid(row=1,column=1, sticky=W)
    global BtnO
    BtnO=Button(frameBtn,text="O",state=DISABLED,width=3,command=lambda:checkWord("Ο"))
    BtnO.grid(row=1,column=2, sticky=W)
    global BtnPi
    BtnPi=Button(frameBtn,text="Π",state=DISABLED,width=3,command=lambda:checkWord("Π"))
    BtnPi.grid(row=1,column=3, sticky=W)
    global BtnP
    BtnP=Button(frameBtn,text="Ρ",state=DISABLED,width=3,command=lambda:checkWord("Ρ"))
    BtnP.grid(row=1,column=4, sticky=W)
    global BtnS
    BtnS=Button(frameBtn,text="Σ",state=DISABLED,width=3,command=lambda:checkWord("Σ"))
    BtnS.grid(row=1,column=5, sticky=W)
    global BtnT
    BtnT=Button(frameBtn,text="T",state=DISABLED,width=3,command=lambda:checkWord("Τ"))
    BtnT.grid(row=1,column=6, sticky=W)
    global BtnY
    BtnY=Button(frameBtn,text="Y",state=DISABLED,width=3,command=lambda:checkWord("Υ"))
    BtnY.grid(row=1,column=7, sticky=W)
    global BtnF
    BtnF=Button(frameBtn,text="Φ",state=DISABLED,width=3,command=lambda:checkWord("Φ"))
    BtnF.grid(row=1,column=8, sticky=W)
    global BtnX
    BtnX=Button(frameBtn,text="X",state=DISABLED,width=3,command=lambda:checkWord("Χ"))
    BtnX.grid(row=1,column=9, sticky=W)
    global BtnPsi
    BtnPsi=Button(frameBtn,text="Ψ",state=DISABLED,width=3,command=lambda:checkWord("Ψ"))
    BtnPsi.grid(row=1,column=10, sticky=W)
    global BtnOmega
    BtnOmega=Button(frameBtn,text="Ω",state=DISABLED,width=3,command=lambda:checkWord("Ω"))
    ...
    global BtnKnowWord
    BtnKnowWord=Button(frameBtn1,text="Βρήκα τη λέξη.",state=DISABLED,command=lambda:KnowWord())
    BtnKnowWord.grid(row=2,column=0, sticky=W)
    global BtnGiveUp
    BtnGiveUp=Button(frameBtn1,text="Παραιτούμαι!\nΕμφάνισε τη λέξη.",state=DISABLED,command=lambda:GiveUp())
...
def StartNewGame():
    ...
    HiddenWord=readWord(WordsList) # κλήση συνάρτησης που διαβάζει - επιλέγει μία λέξη από τη λίστα των λέξεων
    hword=HiddenWord[0]+"-"*(len(HiddenWord)-2)+HiddenWord[len(HiddenWord)-1]
...
WordLabel=Label(root,textvariable=DisplayWord,font=("Ariar", 18),height=1,bg="white",width=30,relief="ridge").pack()
